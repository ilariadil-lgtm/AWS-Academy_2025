...
for p in prodotti:
    if p["prezzo"] >100:
        print(f"Prodotto: {p['nome']}, Prezzo: {p['prezzo']}")

#Calcolare il valore totale dell'inventario (prezzo × quantità per ogni prodotto)
valore_totale_inventario = 0.0
for p in prodotti:
    valore_totale_inventario += p["prezzo"] * p["quantita"]
print(f"Valore totale dell'inventario: {valore_totale_inventario}")
